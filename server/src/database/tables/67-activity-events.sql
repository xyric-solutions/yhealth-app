...
CREATE INDEX idx_activity_events_user_type_date ON activity_events(user_id, type, timestamp DESC);
CREATE INDEX idx_activity_events_timestamp ON activity_events(timestamp DESC);
CREATE INDEX idx_activity_events_confidence ON activity_events(confidence) WHERE confidence < 0.8;
CREATE INDEX idx_activity_events_flags ON activity_events USING GIN (flags);
