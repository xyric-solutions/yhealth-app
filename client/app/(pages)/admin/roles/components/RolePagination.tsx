    ...
    >
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="text-sm text-slate-400">
          Showing <span className="text-emerald-300 font-semibold">{startItem}</span> to{" "}
          <span className="text-emerald-300 font-semibold">{endItem}</span> of{" "}
          <span className="text-white font-semibold">{totalItems}</span> roles
        </div>
        {onItemsPerPageChange && (
        ...
        <div className="flex items-center gap-1.5">
          <AnimatePresence mode="wait">
            {getPageNumbers().map((page, index) => {
              if (page === "ellipsis") {
                return (
                  <div key={`ellipsis-${index}`} className="px-1.5 py-1 text-slate-600">
