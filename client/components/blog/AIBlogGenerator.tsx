...
export function AIBlogGenerator({ onGenerate, disabled }: AIBlogGeneratorProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await api.post<GeneratedBlogData>("/admin/blogs/generate", {
        topic: topic.trim(),
      ...
      const errorMessage = err instanceof ApiError ? err.message : "Failed to generate blog. Please try again.";
      setError(errorMessage);
      toast.error("Error generating blog", { description: errorMessage });
    } finally {
      setGenerating(false);
    }
  };
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="e.g., Getting Started with AI-Powered Health Assessments"
              className="bg-slate-800 border-slate-700 text-white"
              disabled={generating}
            />
            <p className="text-xs text-slate-400">
              Describe what your blog post should be about
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="e.g., Include examples, focus on beginners, mention specific features..."
              rows={3}
              className="bg-slate-800 border-slate-700 text-white"
              disabled={generating}
            />
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                disabled={generating}
              >
                <SelectTrigger id="tone" className="w-full bg-slate-800 border-slate-700 text-white">
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-2">
              <Label htmlFor="length" className="text-white">Content Length</Label>
              <Select
                value={length}
                onValueChange={(val: typeof length) => setLength(val)}
                disabled={generating}
              >
                <SelectTrigger id="length" className="w-full bg-slate-800 border-slate-700 text-white">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-slate-800 border-slate-700 text-white">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="e.g., beginners, health professionals, general audience"
              className="bg-slate-800 border-slate-700 text-white"
              disabled={generating}
            />
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              Include SEO optimization (meta title, description, keywords)
            </Label>
          </div>
